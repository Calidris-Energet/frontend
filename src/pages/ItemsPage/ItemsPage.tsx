...
const ItemsPage: React.FC = () => {
    ...
    const [selectedItemTypeNames, setSelectedItemTypeNames] = useState<
        string[]
    >(searchParams.get("types") ? searchParams.get("types")!.split(",") : []);
    const [selectedItemProducerNames, setSelectedItemProducerNames] = useState<
        string[]
    >(
        searchParams.get("producers")
            ? searchParams.get("producers")!.split(",")
            : []
    );
    ...
    useEffect(() => {
        ...
        if (selectedItemTypeNames.length)
            params.types = selectedItemTypeNames.join(",");
        if (selectedItemProducerNames.length)
            params.producers = selectedItemProducerNames.join(",");
        if (page !== 1) params.page = String(page);

        setSearchParams(params);
    }, [
        name,
        selectedItemTypeNames,
        selectedItemProducerNames,
        page,
        setSearchParams,
    ]);
